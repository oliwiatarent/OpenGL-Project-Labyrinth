...
void Labyrinth::generateCoordinates(int labyrinthNumber, float wallLength, float wallHeight, float wallWidth) {
        ...
        for (i = 0; i < 2; i++) {
            sciany << (szerokosc + 1 + i) * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << (wysokosc) * wallWidth << " "
                << wallLength << " " << wallHeight << " " << (wallWidth + wallLength) << " " << 1 << endl;
                
            sciany << (szerokosc + 1 + i) * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << (wysokosc - 2) * wallWidth << " "
                << wallLength << " " << wallHeight << " " << (wallWidth + wallLength) << " " << 1 << endl;
        }

        for (int j = 0; j < 2; j++) {
            sciany << (szerokosc + i) * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << (wysokosc + j - i) * wallWidth 
                << " " << wallLength << " " << wallHeight << " " << (wallWidth + wallLength) << " " << 0 << endl;
        }

        // Generowanie podłogi
        podlogi << szerokosc * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << (wysokosc - i) * wallWidth 
        ...
        for (int y = 0; y < wysokosc; y++) {
            for (int x = szerokosc - 1; x >= 0; x--) {
                ...
                sciany << k * (wallLength + wallGap) + wallLength << " " << labyrinthNumber * wallHeight << " " << y * (wallWidth) << " " << 
                        wallLength << " " << wallHeight << " " << wallLength << " " << 1 << endl;

                // Ściany pioniowe z punktu widzenia obserwatora
                if (sciana[x][y][0]) {
                    sciany << k * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << y * wallWidth << " " << 
                        wallLength << " " << wallHeight << " " << wallWidth << " " << 1 << endl;
                }
                ...
                if (y == wysokosc - 1 && sciana[x][y][2]) {
                    sciany << k * (wallLength + wallGap) << " " << labyrinthNumber * wallHeight << " " << (y + 1) * wallWidth << " "
                        << wallLength << " " << wallHeight << " " << (wallWidth + wallLength) << " " << 1 << endl;
                }
                ...
                if (x == szerokosc - 1 && sciana[x][y][1]) {
                    if (y != 0) // Nie generuj ściany: wejście do labiryntu 
                        sciany << (k * (wallLength + wallGap)) - wallLength - wallGap << " "  << labyrinthNumber * wallHeight << " " 
